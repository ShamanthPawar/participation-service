...
public class ParticipationController {
	...
	@PutMapping("/updateStatus/{participationId}")
	@CrossOrigin(origins = "http://localhost:4200")
    ...
    public List<Participant> getApprovedParticipations(@PathVariable("approved") String approvedStatus){
        return  ps.getParticipantsByStatus(approvedStatus);
    }

    @GetMapping("/getDeclinedParticipations/{declined}")
    public  List<Participant> getDeclinedParticipations(@PathVariable("declined") String declinedStatus){
        return ps.getParticipantsByStatus(declinedStatus);
    }

    @GetMapping("/getPendingParticipations/{pending}")
    public  List<Participant> getPendingParticipations(@PathVariable("pending") String pendingStatus){
        return  ps.getParticipantsByStatus(pendingStatus);
    }
};
